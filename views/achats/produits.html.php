		...
		<div class="d-flex flex-wrap justify-content-center">
			<?php foreach ($produits as $produit) {
				if ($produit->getStock() > 0) {
					?>
					<div class="card m-5 plantesResults relative-div-achats" style="width: 18rem;"
						data-nom="<?= $produit->getNomProduit(); ?>">
						<div class="img-card">
							<img src="<?= $cheminDossier . $produit->getImage() ?>" class="card-img-top"
								alt="<?= $produit->getNomProduit(); ?>" />
						</div>
						<div class="card-body text-center">
							<h5 class="card-title">
								<?= $produit->getNomProduit(); ?>
							</h5>
							<a href="<?= addLink('home', 'details', $produit->getId()) ?>"
								class="btn btn-warning change-btn">Voir l'article en détail</a>

						</div>
						<span class="ajouter_panier_icon" data-url="<?= addLink('panier', 'addToCart'); ?>"
							data-id="<?= $produit->getId() ?>" data-nom="<?= $produit->getNomProduit() ?>"
							data-prix="<?= $produit->getPrixProduit(); ?>" data-image="<?= $produit->getImage(); ?>">
				...
				<?php } else { ?>
					<div class="card m-5 plantesResults relative-div-achats" style="width: 18rem;"
						data-nom="<?= $produit->getNomProduit(); ?>">
						<div class="img-card">
							<img src="<?= $cheminDossier . $produit->getImage() ?>" class="card-img-top img-hors-stock"
								alt="<?= $produit->getNomProduit(); ?>" />
